#!/bin/bash
if [ "$#" -ne 3 ]; then
  echo "Usage: $0 /home/.../XXX.riproject TileID ContainerSuffix" >&2
  exit 1
fi
#run semantic segmentation
docker run -d --rm -v $1:/data/ --name=semseg$3 tls2trees:latest run.py -p /data/extraction/downsample/$2.downsample.ply \
...
echo "semantic segmentation started, name=semseg$3"
# TODO: TEMP: start up monitoring of docker container to view MEM and CPU usage
./logContainerStats.sh semseg$3
#wait for instance segmentation to finish
docker wait semseg$3
echo "starting instance segmentation"
#run instance segmentation for individual trees
docker run -d --rm -v $1:/data/ --name=instseg$3 tls2trees:latest points2trees.py -t /data/clouds/SemanticSeg/$2.downsample.segmented.ply \
...
echo "instance segmentation started, name=instseg$3"
# TODO: TEMP: start up monitoring of docker container to view MEM and CPU usage
./logContainerStats.sh instseg$3
#wait for instance segmentation to finish
docker wait instseg$3
